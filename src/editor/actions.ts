...
export function buildActions(): Record<string, Action> {
  const list: Action[] = [
    ...
    { id: 'file.voidPng', label: 'Download editable picture (.void.png)', run: () => import('./io').then(m => m.exportVoidPng()), enabled: hasDoc, keywords: 'save editable file layers png preview' },
    ...
    { id: 'board.duplicate', label: 'Duplicate board', hotkey: 'Ctrl+Alt+D', run: () => { const st = s(); const id = st.activeFrameId ?? st.doc?.frames?.[0]?.id; if (id) st.duplicateFrame(id, 'right') }, enabled: () => !!s().doc?.frames?.length, keywords: 'artboard copy clone' },
    { id: 'board.empty', label: 'New board beside this one', run: () => { const st = s(); const id = st.activeFrameId ?? st.doc?.frames?.[0]?.id; if (id) st.duplicateFrame(id, 'right', true) }, enabled: () => !!s().doc?.frames?.length, keywords: 'artboard add blank' },
    { id: 'board.organise', label: 'Organise boards', run: () => s().organiseFrames(), enabled: () => (s().doc?.frames?.length ?? 0) > 1, keywords: 'artboards tidy grid arrange' },
    ...
    { id: 'formats.sync', label: 'Update formats from master', run: () => ops.syncFormats(), enabled: () => !!s().doc?.frames?.some(f => f.linkedFrom), keywords: 'linked variants resize sync key visual' },
    { id: 'formats.relay', label: 'Re-lay this format from master', run: () => ops.relayFormat(), enabled: () => !!s().doc?.frames?.some(f => f.linkedFrom), keywords: 'reset layout' },
    ...
    { id: 'help.bug', label: 'Report a bug…', run: () => openBugReport('menu'), keywords: 'problem broken error issue crash' },
    { id: 'help.learn', label: 'Learn Voidcanvas', run: () => window.open('/learn', '_blank', 'noopener'), keywords: 'guides tutorials docs help how to' },
    { id: 'help.guide', label: 'Editor guide', run: () => window.open('/learn/editor-tour', '_blank', 'noopener'), keywords: 'tour tutorial layout' },
    { id: 'help.shortcutsRef', label: 'All shortcuts (web page)', run: () => window.open('/learn/keyboard-shortcuts', '_blank', 'noopener'), keywords: 'keys keyboard reference print' },
    { id: 'help.blog', label: 'Blog', run: () => window.open('/blog', '_blank', 'noopener'), keywords: 'news updates' },
    { id: 'help.about', label: 'About Voidcanvas', run: () => window.open('/about', '_blank', 'noopener'), keywords: 'company motionplay labs' },
  ]
